    ...
    def prepare_for_inference(self):
        ...
        if self.model_config["quantization"] == "int8":
            ...
            if self.model_config["gpuType"] == "T4":
                engine_args = AsyncEngineArgs(model=self.model_config["huggingface_name"], gpu_memory_utilization=0.97, tensor_parallel_size=self.model_config["gpuCount"])
            else:
                engine_args = AsyncEngineArgs(model=self.model_config["huggingface_name"], tensor_parallel_size=self.model_config["gpuCount"])
