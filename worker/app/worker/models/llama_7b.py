...
class Llama7B(RegisteredModel):
    ...
    def prepare_for_inference(self):
        if self.model_config["quantization"] == "int8":
            raise NotImplementedError("OpenLlama Models do not yet support 8bit-quantization. We're working on it!")
    ...
    def generate_stream(self, text_input: str, conversation_history: List, sample: bool = True, top_p: float = 0.8, top_k: int = 500, temperature: float = 0.9, max_length: int = 2048):
        ...
        sampling_params = SamplingParams(max_tokens=max_length, top_p=top_p, temperature=temperature, stop=self.model_config["stopTokens"])
