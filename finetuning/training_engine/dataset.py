...
def preprocess(conversations: Sequence[Sequence[dict]], tokenizer: transformers.PreTrainedTokenizer, conversation_template: str, max_tokens: int) -> Dict:
    ...
    for conv in conversations:
        ...
        nontokenized_conv = tokenizer.apply_chat_template(current_conv, chat_template=conversation_template, max_length=max_tokens, truncation=True, tokenize=False)

        print("nontokenized", nontokenized_conv)

        loss_aware_tokenized_conv = [-100] * len(tokenized_conv)

        for sublist in tokenized_responses:
            for index, value in enumerate(tokenized_conv):
                if value in sublist:
                    loss_aware_tokenized_conv[index] = value
                    if index < len(tokenized_conv) - 1 and tokenized_conv[index + 1] not in sublist:
                        loss_aware_tokenized_conv[index + 1] = tokenized_conv[index + 1]

        all_input_ids.append(torch.LongTensor(tokenized_conv))
        all_label_ids.append(torch.LongTensor(loss_aware_tokenized_conv))
